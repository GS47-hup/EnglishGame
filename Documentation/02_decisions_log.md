...
| 2026-04-16 | Visual Audio UX Over Textual Hints | Removed explicit text descriptions (e.g. "___ Going bowling") from listening exams and rely purely on imagery and audio. This removes "Training Wheels", preventing students from guessing via reading comprehension and enforcing pure listening skills akin to KET/PET exams. |
| 2026-04-16 | Resilient Audio Engine (Web Audio API) | Migrated from HTML5 `<audio>` tags to Web Audio API (`AudioContext`). This bypasses "Communication Ducking" on mobile devices (iOS/Android) during active Google Meet calls by treating audio as processed buffers rather than media streams. |
